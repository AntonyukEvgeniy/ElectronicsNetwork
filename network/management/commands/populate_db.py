    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                suppliers = []
                for _ in range(50):
                    supplier_type = random.choice([NetworkNode.DISTRIBUTOR, NetworkNode.DEALER])
                    parent = random.choice(factories if supplier_type == NetworkNode.DISTRIBUTOR
                                           else factories + suppliers)

                    supplier = NetworkNode.objects.create(
                        name=f"{'Дистрибьютор' if supplier_type == NetworkNode.DISTRIBUTOR else 'Дилер'} {fake.company()}",
                        type=supplier_type,
                        email=fake.email(),
                        country=fake.country(),
                        city=fake.city(),
                        ...
                        building=str(fake.building_number()),
                        parent=parent,
                        debt=Decimal(random.uniform(0, 1000000)).quantize(Decimal("0.01"))
                    )
                ...
                for _ in range(10000):
                    ...
                    product = Product.objects.create(
                        ...
                        price=Decimal(random.uniform(100, 100000)).quantize(Decimal("0.01"))
                    )
            ...
            self.stdout.write(self.style.ERROR(f'Ошибка при заполнении базы данных: {str(e)}'))